...
export function JobCompletion({shift,save,onDone}:{shift:Shift;save:(op:Omit<PendingOperation,'uid'|'captured_at'>)=>Promise<void>;onDone:()=>void}) {
 ...
 const anyDevice=assigned.some((_,index)=>stockDeviceNeeded(actionFor(index)));const anySim=assigned.some((_,index)=>simNeeded(actionFor(index)));
 ...
 async function submit(e:FormEvent){e.preventDefault();setError('');try{
  const unitRecords:UnitCompletion[]=assigned.map((unit,index)=>{const action=actionFor(index);return{...unit,...(unit.job_type==='inspection'?{inspection_action:inspectionActions[index]}:{}),...(deviceNeeded(action)?{...(stockDeviceNeeded(action)?{device_model:model}:{}),device_imei:clean(imeis[index])}:{}),...(simNeeded(action)?{sim_number:clean(sims[index])}:{})}});
  ...
  await save({id:shift.id,kind:'job-completed',shift_id:shift.id,job_type:shift.job_type,inspection_action:shift.job_type==='inspection'?inspectionActions[0]:undefined,unit_count:shift.unit_count,unit_records:unitRecords,device_model:unitRecords.some(row=>stockDeviceNeeded(row.job_type))?model:'',device_imeis:di,sim_numbers:sn,quantity:unitRecords.filter(row=>stockDeviceNeeded(row.job_type)).length,sim_count:sn.length,sim_provider:sn.length?simProvider:undefined,customer_ref:shift.company_name||shift.customer_name||shift.site_name,vehicle_ref:assigned.map(row=>row.vehicle_number).filter(Boolean).join(' · '),notes:notes.trim(),completion_latitude:location.latitude,completion_longitude:location.longitude,completion_accuracy_m:location.accuracy_m});onDone();
 ...
 return <form className="completion-card" onSubmit={submit}><div className="completion-heading"><div><span className="eyebrow">JOB COMPLETION</span><h3>{jobLabel(shift.job_type)} · {shift.unit_count} unit{shift.unit_count===1?'':'s'}</h3></div><CheckCircle2/></div>
  ...
  <div className="identifier-grid">{assigned.map((unit,index)=>{const action=actionFor(index);const needsDevice=deviceNeeded(action);const needsSim=simNeeded(action);return <div className="unit-record" key={index}><strong>Unit {index+1} · {unit.vehicle_number||'No plate'}</strong><small>Assigned: {jobLabel(unit.job_type)}</small>
   ...
   {!needsDevice&&!needsSim&&<div className="notice info">{unit.job_type==='device_removal'?'Scan the removed device IMEI. No device or SIM stock is deducted.':'No scan required for a check-only inspection.'}</div>}
  </div>})}</div>
...
function inventoryText(actions:string[]){const devices=actions.filter(stockDeviceNeeded).length;const sims=actions.filter(simNeeded).length;return devices||sims?`Inventory deduction: ${devices} device${devices===1?'':'s'} and ${sims} SIM${sims===1?'':'s'}.`:'No inventory is deducted for removals or check-only inspections.'}
