...
export const ROLES = ['master_admin', 'owner', 'admin', 'manager', 'accountant', 'hr', 'it', 'technician'] as const;
...
export interface Movement { id: string; technician_id: string; technician_name: string; type: 'received' | 'installed' | 'sim-used' | 'adjustment'; device_model: DeviceModel | ''; quantity: number; sim_count: number; device_imeis?: string[]; quantity_delta?: number; sim_delta?: number; sim_provider?: SimProvider; timestamp: string; notes: string; }
...
export const roleLabel = (role: Role) => ({ master_admin: 'Master administrator', owner: 'Owner', admin: 'Administrator', manager: 'Manager', accountant: 'Accountant', hr: 'Human resources', it: 'IT support', technician: 'Technician' }[role]);
...
export function completionActor(installation:Installation){return installation.completion_source==='admin_override'?`${installation.completed_by_name||'Administrator'} (${roleLabel(installation.completed_by_role||'admin')})`:`${installation.technician_name||'Technician'} (Technician)`}

export interface LeaveRequest { id:string; technician_id:string; technician_name:string; from:string; to:string; reason:string; status:'pending'|'approved'|'rejected'; requested_at:string; reviewed_at?:string; reviewed_by?:string; reviewer_note?:string; }
export interface CompanyHoliday { id:string; date:string; name:string; created_at:string; }
export interface AttendanceCorrection { id:string; technician_id:string; date:string; requested_clock_in?:string; requested_clock_out?:string; reason:string; status:'pending'|'approved'|'rejected'; requested_at:string; reviewed_at?:string; reviewed_by?:string; }
