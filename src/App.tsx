...
import { ensureInventory, ensureProfile, legacyQueueCount, mapAccount, mapAttendance, mapInstallation, mapMovement, mapShift, mapWorkBreak, mapWorkSession, observe, observeProfile, observeStockAlertSettings, pending, queueOperation, saveStockAlertThreshold, syncOperations } from './lib/data';
...
export default function App() {
 ...
 const [workSessions,setWorkSessions]=useState<WorkSession[]>([]);const [workBreaks,setWorkBreaks]=useState<WorkBreak[]>([]);const [stockAlerts,setStockAlerts]=useState<StockAlertSettings>({threshold:2,clock_in_time:'09:00',clock_out_time:'18:00',late_grace_minutes:5,max_break_minutes:60});
 ...
 useEffect(()=>{
  ...
  const subs=[
   observeStockAlertSettings(setStockAlerts,fail),
 ...
 const nav=[{id:'overview',label:'Today',icon:LayoutDashboard},{id:'attendance',label:'Today’s jobs',icon:MapPin},{id:'inventory',label:'Inventory',icon:Package},{id:'installations',label:'History & report',icon:BarChart3},{id:'settings',label:'Settings',icon:SettingsIcon}];
 return <div className={'app-shell '+platformClass}><aside className="sidebar"><a className="brand-logo" href="#" onClick={e=>{e.preventDefault();setTab(isStaff?'admin':'overview');}}><img src="/securetrack-logo.png" alt="SecureTrack"/></a><div className="workspace-label">OPERATIONS <span>LIVE</span></div><nav>{isStaff?adminNav.map(n=><button key={n.id} className={adminSection===n.id?'active':''} onClick={()=>{setTab('admin');setAdminSection(n.id)}}><n.icon size={19}/>{n.label}{adminSection===n.id&&<span className="nav-dot"/>}</button>):nav.map(n=><button key={n.id} className={tab===n.id?'active':''} onClick={()=>setTab(n.id as Tab)}><n.icon size={19}/>{n.label}{tab===n.id&&<span className="nav-dot"/>}</button>)}</nav><div className="sidebar-bottom"><div className="user-block"><div className="avatar">{profile.photoDataUrl?<img src={profile.photoDataUrl} alt=""/>:(profile.displayName||profile.email).slice(0,2).toUpperCase()}</div><div><strong>{profile.displayName||profile.email.split('@')[0]}</strong><small>{roleLabel(profile.role)}</small></div><button aria-label="Sign out" onClick={()=>signOut(auth)}><LogOut size={18}/></button></div></div></aside>
 <div className="main-shell"><header className="topbar"><span>SecureTrack <span className="slash">/</span> <strong>{isStaff?adminNav.find(n=>n.id===adminSection)?.label||'Operations':nav.find(n=>n.id===tab)?.label||nav[0].label}</strong></span><div className="top-actions"><button className="text-button desktop-signout" onClick={()=>signOut(auth)} aria-label="Sign out of account"><LogOut size={16}/></button><span className={'connection '+(online?'':'offline')}>{online?<Wifi size={14}/>:<WifiOff size={14}/>}<span>{online?'Live':'Offline'}</span></span><span className="date-label">{new Date(now).toLocaleDateString('en-GB',{timeZone:TIME_ZONE,day:'numeric',month:'short',year:'numeric'})}</span><button className="top-profile" aria-label="Open my settings" onClick={()=>{if(isStaff){setTab('admin');setAdminSection('settings')}else setTab('settings')}}>{profile.photoDataUrl?<img src={profile.photoDataUrl} alt=""/>:(profile.displayName||profile.email).slice(0,2).toUpperCase()}</button></div></header>
 <main className="workspace"><div className={'mobile-nav '+(mobileNavHidden?'hidden':'')}>{(isStaff?adminNav:nav).map(n=><button key={n.id} onClick={()=>{if(isStaff){setTab('admin');setAdminSection(n.id as AdminSection)}else setTab(n.id as Tab)}} className={(isStaff?adminSection===n.id:tab===n.id)?'active':''}><n.icon size={17}/><span>{n.label}</span></button>)}</div>
 ...
 {isStaff?<AdminDashboard section={adminSection} onSectionChange={setAdminSection} loginLogs={loginLogs} workSessions={workSessions} workBreaks={workBreaks} currentUid={user.uid} currentRole={profile.role} technicians={technicians} installations={allInstallations} movements={movements} accounts={accounts} shifts={shifts} attendance={attendance} stockAlerts={stockAlerts} onSaveStockAlerts={saveStockAlertThreshold} now={now}/>:<>
 <div className="page-heading"><div><span className="eyebrow">{tab==='overview'?'TODAY AT A GLANCE':'MY FIELD WORKSPACE'}</span><h1>{tab==='overview'?`Good to see you, ${signedInName}`:tab==='inventory'?'Stock on hand':tab==='installations'?'History & report':tab==='attendance'?'Today’s jobs':'My settings'}<span className="accent">.</span></h1><p>{tab==='overview'?'Here is today’s field plan.':tab==='inventory'?'Issued stock updates automatically as jobs are completed.':tab==='installations'?'Review and download your completed work history.':tab==='attendance'?'Only jobs scheduled for today appear here.':'Manage your account details.'}</p></div></div>
 ...
 {tab==='installations'&&<TechnicianHistory profile={profile} shifts={ownShifts} installations={ownInstallations} attendance={attendance} movements={ownMoves} accounts={accounts} loginLogs={loginLogs} workSessions={workSessions} workBreaks={workBreaks}/>}
 {tab==='attendance'&&(todayWorkSession?<AttendancePanel shifts={todayShifts} attendance={attendance} installations={ownInstallations} save={save} now={now}/>:<div className="notice warning">Clock in above to open today’s assigned jobs.</div>)}
