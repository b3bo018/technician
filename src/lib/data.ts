...
export function observeStockAlertSettings(callback:(settings:StockAlertSettings)=>void,error:(e:Error)=>void){return onSnapshot(doc(db,'settings','stock_alerts'),snap=>callback({threshold:Math.max(0,Math.min(10000,Number(snap.data()?.threshold??2)||0))}),error)}
...
export async function saveStockAlertThreshold(threshold:number){const value=Math.max(0,Math.min(10000,Math.floor(threshold)));await setDoc(doc(db,'settings','stock_alerts'),{threshold:value,updated_at:serverTimestamp()},{merge:true});}
