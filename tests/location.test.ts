...
test('GPS failure retries with lower accuracy and preserves reported accuracy',async()=>{
 const modes:boolean[]=[];setup((ok:any,fail:any,options:any)=>{modes.push(options.enableHighAccuracy);if(options.enableHighAccuracy)fail({code:3});else ok({coords:{latitude:25,longitude:55,accuracy:400}});});
 assert.deepEqual(await captureLocation(),{latitude:25,longitude:55,accuracy_m:400});assert.deepEqual(modes,[true,false]);
});
