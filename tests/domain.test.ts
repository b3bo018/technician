...
test('inspection completion supports checks and replacement combinations',()=>{const base:any={kind:'job-completed',shift_id:'s',job_type:'inspection',customer_ref:'C',device_model:'',quantity:0,sim_count:0,device_imeis:[],sim_numbers:[],completion_latitude:25.2,completion_longitude:55.3,completion_accuracy_m:8};assert.doesNotThrow(()=>validateOperation({...base,inspection_action:'check_only'}));assert.doesNotThrow(()=>validateOperation({...base,inspection_action:'sim_change',sim_count:1,sim_provider:'du',sim_numbers:['8997112212789413817']}));assert.throws(()=>validateOperation({...base,inspection_action:'device_change'}));});
