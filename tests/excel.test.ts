...
test('Excel report leads with simple customer, device and SIM usage',async()=>{
 ...
 assert.equal(restored.worksheets.length,7);assert.ok(restored.getWorksheet('Summary'));assert.ok(restored.getWorksheet('Jobs'));const usage=restored.getWorksheet('Customer usage')!;assert.equal(usage.rowCount,2);assert.equal(usage.getCell('C2').value,'Acme');assert.equal(usage.getCell('F2').value,'FMC920');assert.equal(usage.getCell('G2').value,1);assert.equal(usage.getCell('H2').value,'Etisalat');assert.equal(usage.getCell('I2').value,1);assert.ok(restored.getWorksheet('Technician inventory'));assert.ok(restored.getWorksheet('Shift attendance'));
});
